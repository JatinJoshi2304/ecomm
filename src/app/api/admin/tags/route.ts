...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing) {
      return NextResponse.json(errorResponse("VALIDATION_FAILED", RESPONSE_MESSAGES.STATUS_CODES.BAD_REQUEST, "Tag already exists"), { status: RESPONSE_MESSAGES.STATUS_CODES.BAD_REQUEST });
    }

    const tag = await Tag.create({ name, type });
    return NextResponse.json(successResponse("CREATE", tag, RESPONSE_MESSAGES.STATUS_CODES.CREATED), { status: RESPONSE_MESSAGES.STATUS_CODES.CREATED });
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse("FETCH", tags, RESPONSE_MESSAGES.STATUS_CODES.SUCCESS), { status: RESPONSE_MESSAGES.STATUS_CODES.SUCCESS });
