...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingColor) {
      return NextResponse.json(errorResponse("VALIDATION_FAILED", RESPONSE_MESSAGES.STATUS_CODES.BAD_REQUEST, "Color already exists"), { status: RESPONSE_MESSAGES.STATUS_CODES.BAD_REQUEST });
    }

    const color = await Color.create({ name, hexCode });
    return NextResponse.json(successResponse("CREATE", color, RESPONSE_MESSAGES.STATUS_CODES.CREATED), { status: RESPONSE_MESSAGES.STATUS_CODES.CREATED });
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(successResponse("FETCH", colors, RESPONSE_MESSAGES.STATUS_CODES.SUCCESS), { status: RESPONSE_MESSAGES.STATUS_CODES.SUCCESS });
